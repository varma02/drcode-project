...
import { BadRequestError, BaseError, ErrorCode, FieldsInvalidError } from './errors';
  ...
  return async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    try {
      ...
      if (err instanceof BaseError) {
        ...
        if (
          err.message.match(/Found.+for field.+with record.+but/gmi) ||
          err.message.match(/Expected a single result output when using the ONLY keyword/gmi)
        ) {
